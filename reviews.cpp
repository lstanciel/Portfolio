...
void readKeywordWeights(istream &input, vector<string> &keywords, vector<double> &weights) {
    //Creates empty variables to be read into input
    double number;
    string word;
...
double reviewScore(const vector<string> &reviewWords, const vector<string> &keywords, const vector<double> &weights) {
    vector<string> copyRW = reviewWords;

    double totalScore;
    preprocessReview(copyRW);

    for(int i = 0; i < copyRW.size(); ++i){
        totalScore += wordWeight(copyRW[i], keywords, weights);
    }
